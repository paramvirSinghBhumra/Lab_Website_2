...
function loadStatsPage(){
	var winsCU = 0;
	var lossesCU = 0;
	for(counter = 2; counter<document.getElementById("stats_table").rows.length; counter++){
		var homeScore = document.getElementById("stats_table").rows[counter].cells[2].innerHTML;
		var oppScore = document.getElementById("stats_table").rows[counter].cells[3].innerHTML;

		if(parseInt(homeScore)>parseInt(oppScore)){
			winsCU++;
			document.getElementById("stats_table").rows[counter].cells[4].innerHTML = "CU";
		}
		else{
			lossesCU++;
			document.getElementById("stats_table").rows[counter].cells[4].innerHTML = document.getElementById("stats_table").rows[counter].cells[1].innerHTML;
		}
	}

	document.getElementById("wins").innerHTML = winsCU;
	document.getElementById("losses").innerHTML = lossesCU;
}
...
function loadPlayersPage(){
	...
	for (var i = 0; i < players.length; i++) {
		playerNames+='<button class="dropdown-item" onclick="switchPlayers('+i+')">'+players[i].name+'</button><br>';
	}
	document.getElementById("player_selector").innerHTML = playerNames;
}
